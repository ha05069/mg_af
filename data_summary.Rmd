...
The final joined data consists of 38,521 observations of 5824 individual patients (as determined by unique patient identity numbers), across 6779 distinct episodes (as defined by unique CCHIC episode identity numbers).  There were 6776 distinct episodes defined using ICIP episode identity numbers.  
...
```{r, results='asis', echo=FALSE}
# table for missing data per Patient:
...
There were 87 episodes labelled as planned medical admissions, 2506 planned surgical episodes and 4186 episodes of admission type 'Other'. 
...
adm_type_row <- tibble(
  ...
  count = c(272, 8625, 30634),
  prop = count/39531 * 100)
...
Severity of illness was estimated by admission APACHE score.  This was absent for 50 patients.  The median APACHE score per episode was 16 (12 - 20).  This was the same for males and females.  
...
```{r, results='asis', echo=FALSE}
sex <- c("Male", "Female")
cpr <- c("Yes", "No", "Missing")
count <- c(78, 3342, 1)


cpr <- matrix(c(78, 3342, 1, 107, 3249, 1), ncol = 3, byrow = T)
...
# nb equates to 6778 episodes because of the 1 episode with NA for sex
```

Unit mortality 9.34% across all selected episodes.  The mean serum magnesium on admission for surviving patients was 0.79 mmol/L, versus 0.87 mmol/L for those who died.
...
For reference the join was defined such that magnesium measurements associated with the episode ID were joined to that episode, occurring up to 12 hours prior to the episode start time identified in the CCHIC data.  This was to allow initial magnesium measurements occurring prior to admission but still influencing decisions taken during the admission to be captured.  For reference, 98% of episodes had their first serum magnesium measurement recorded within 24 hours of the episode start.  
...
The mean serum magnesium value across the duration of the episode was 0.86 mmol/L (sd, 0.15).  

The mean change in serum magnesium value across each episode was -0.12 mmol/L (sd, 0.27).
...
The median number of administrations per episode was 1 (0 - 2), with a range of 0-78 administrations per episode.
...
There were 641 episodes which started in AF.  AF events were defined as rhythm changes from sinus rhythm or "other" to AF within the magnesium window.  510 episodes contained at least one AF event.  1388 magnesium windows contained at least one AF event.  

The mean serum magnesium at the start of the magnesium window for windows with no AF events was 0.92 mmol/L.
...
table1 <- tibble(
  ...
  Summary = c("64 +/- 23",
             "Female: 51.1",
             "2.3 +/- 4",
             "Medical: 87 (1.28%), Surgical: 2506 (37%), Other: 4186 (61.7%)",
             "16 +/- 8",
             "2.81",
             "9.34",
             "4.46 (0.36)",
...
# here the unit of summary is each mg window, n = 39,531)
...
table2 <- tibble(
  Variable = c("Number receiving Magnesium Administration (%)",
               ...
               "Median (IQR) time to Atrial Fibrillation Event (hours)",
               "Median (IQR) time to Magnesium Administration (hours)"),
  ...
  Summary = c("15,065 (38.1)",
              "6 (6)",
              "0 - 15 (1.49% with > 1 Magnesium Administration)",
              "0.93 (0.22)",
              "2836 (7.2)",
              "1388 (3.5)",
              "385 (2.6)",
